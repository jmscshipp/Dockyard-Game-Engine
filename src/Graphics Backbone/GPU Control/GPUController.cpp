...
void GPUController::PrivInitDirect3D(const HWND& window, int windowWidth, int windowHeight)
{
	...
	// CREATE FRONT FACE CULL SOLID RASTERIZER STATE
	rd.FillMode = D3D11_FILL_SOLID;
	...
	md3dDevice->CreateRasterizerState(&rd, &mRasterizerFrontcullSolid);
...
void GPUController::PrivSetRasterizerFrontcullSolid()
{
	md3dImmediateContext->RSSetState(mRasterizerFrontcullSolid);
}