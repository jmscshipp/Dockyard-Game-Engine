...
Shader::Shader(WCHAR* filename, LPCSTR vsModel, LPCSTR psModel)
...
void Shader::privBuildShaders(WCHAR* filename, LPCSTR vsModel, LPCSTR psModel)
{
	pVSBlob = nullptr;
...
void Shader::CreateInputLayout(D3D11_INPUT_ELEMENT_DESC* layoutdesc, UINT size)
{
...
void Shader::SetContext(ID3D11DeviceContext* devcon)
{
...
void Shader::SetToContext_VS_PS_InputLayout()
{
	assert( mContext != nullptr && "ERROR: Context not set.");
