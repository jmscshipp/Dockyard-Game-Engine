...
float ClosestVal(float val, float compare0, float compare1, float compare2)
{
    float dif0 = abs(val - compare0);
    float dif1 = abs(val - compare1);
    float dif2 = abs(val - compare2);

    if (dif0 < dif1)
        return (dif0 < dif2) ? compare0 : compare2;
    else
        return (dif1 < dif2) ? compare1 : compare2;
}
...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    //float4 color0 = float4(0.5000000000f, 0.647058845f, 0.000000000f, 1.000000000f);
    //float4 color1 = float4(0.2000000000f, 0.411764741f, 0.705882370f, 1.000000000f);
    //float4 color2 = float4(0.700000000f, 0.000000000f, 0.000000000f, 1.000000000f);
	
    float4 clampedColor;
    clampedColor.x = ClosestVal(result.x, Color0.x, Color1.x, Color2.x);
    clampedColor.y = ClosestVal(result.y, Color0.y, Color1.y, Color2.y);
    clampedColor.z = ClosestVal(result.z, Color0.z, Color1.z, Color2.z);
    clampedColor.w = 1.0f;
    return clampedColor;
}
